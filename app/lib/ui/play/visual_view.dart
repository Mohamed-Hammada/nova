...
class TokenArt extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      SizedBox(width: size, height: size, child: CustomPaint(painter: _TokenPainter(token)));
}
...
class _TokenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
      path,
      Paint()..shader = RadialGradient(center: const Alignment(-0.4, -0.45), radius: 0.95, colors: [shade(color, 0.45), color, shade(color, -0.35)], stops: const [0, 0.45, 1]).createShader(Rect.fromCircle(center: c, radius: r)),
    );
    canvas.drawOval(Rect.fromCenter(center: c + Offset(-r * 0.3, -r * 0.4), width: r * 0.6, height: r * 0.3), Paint()..color = Colors.white.withValues(alpha: 0.5));
  }
...
class _Group extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cells = [for (var r = 0; r < math.max(cols, rows); r++) for (var c = 0; c < math.max(cols, rows); c++) (r, c)]..shuffle(rng);
    return SizedBox(
      width: size,
      height: size,
      child: Stack(
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Row(
        ...
        children: [
          for (final t in v.shown) _Carriage(size: item, child: TokenArt(t, size: item * 0.8)),
          _Carriage(size: item, child: Text('?', style: novaText(item * 0.55, weight: 800, color: const Color(0xFF7C6CF2)))),
        ],
      ),
    );
  }
    ...
    Widget tower(int h, {bool mystery = false}) => Padding(
          ...
          child: Column(
            ...
            children: mystery
                ...
                : [for (var i = 0; i < h; i++) Container(width: block, height: block * 0.92, margin: EdgeInsets.only(top: block * 0.06), decoration: BoxDecoration(borderRadius: BorderRadius.circular(block * 0.18), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [shade(const Color(0xFF28B8E8), 0.35), const Color(0xFF1E88C8)])))],
          ),
        );
    return Directionality(
      textDirection: TextDirection.ltr,
        ...
        child: ClipRect(
          child: OverflowBox(
            ...
            child: SizedBox(
              width: size * 1.5,
              height: size * 2.0,
              child: CustomPaint(painter: CharacterPainter(kind: kindOf(who), pose: emotionPose(emotion))),
            ),
          ),
        ),
      );
...
class _Scene extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: Stack(
          children: [
            Positioned.fill(child: CustomPaint(painter: CharacterPainter(kind: kindOf(v.who), pose: actionPose(v.action)))),
            if (v.prop != null) Positioned(right: 0, bottom: size * 0.05, child: PicArt(v.prop!, size: size * 0.36)),
            if (v.action == Act.sleeping)
              Positioned(right: size * 0.12, top: size * 0.04, child: Text('z z', style: novaText(size * 0.14, weight: 800, color: const Color(0xFF7C6CF2)))),
          ],
        ),
      );
}
