...
Widget gameArt(Game game, double w, String language) {
  Widget pics(List<Pic> list) => Row(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [for (final p in list) PicArt(p, size: w * 0.28)],
  );
  Widget tokens(List<Token> list) => Row(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [for (final t in list) TokenArt(t, size: w * 0.22)],
  );
  Widget letters(String text) => Text(
    text,
    style: novaText(w * 0.26, weight: 800, color: Colors.white).copyWith(shadows: const [Shadow(blurRadius: 6, color: Color(0x66000000))]),
  );
  final id = game.id;
  final Widget art = switch (game.mechanicId) {
    ...
    'match-symbol-to-quantity' => Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        NumeralBadge(numeral(3, language), size: w * 0.24),
        const SizedBox(width: 8),
        PicArt(Pic.star, size: w * 0.26),
      ],
    ),
    'catch-target' =>
      id.contains('lit')
          ? Stack(
              ...
              children: [
                PicArt(Pic.balloon, size: w * 0.4),
                letters(language == 'ar' ? 'ب' : 'a'),
              ],
            )
          : pics([Pic.balloon, Pic.balloon]),
    'pattern-continue' =>
      id.contains('builder')
          ? VisualView(const TowersVisual([1, 2, 3], withGap: false), size: w * 0.4, language: language)
          : tokens(const [Token(Shape.circle, Hue.red), Token(Shape.circle, Hue.blue), Token(Shape.circle, Hue.red)]),
    ...
    'number-line-place' => Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        for (final n in [0, 5, 10])
          Padding(
            padding: const EdgeInsets.all(4),
            child: NumeralBadge(numeral(n, language), size: w * 0.18),
          ),
      ],
    ),
    ...
    'emotion-match' => Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        FaceArt(Who.bunny, Emotion.happy, size: w * 0.34),
        FaceArt(Who.fox, Emotion.sad, size: w * 0.34),
      ],
    ),
    ...
    'sound-match' => Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        PicArt(Pic.sun, size: w * 0.3),
        letters(language == 'ar' ? 'ش' : 's'),
      ],
    ),
    ...
    'read-and-match' => Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        letters(language == 'ar' ? 'شمس' : 'sun'),
        PicArt(Pic.sun, size: w * 0.26),
      ],
    ),
    _ => pics([Pic.star]),
  };
